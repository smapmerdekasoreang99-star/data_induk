...
let D = { siswa: [], guru: [], rombel: [], mapel: [], tugas: [], tahun: [], jabatan: [],
          jenis: [], piket: [], parkiran: [], piketUnit: [], ttd: {}, jamKerja: [], jamKerjaGuru: [],
...
function dataContoh() {
  ...
  D.guru = [
    ...
    ['G005','5','Ujang Supriatna','','Tenaga Kependidikan','L','2018-07-16'],
    ['G006','6','Yani Mulyani','','Tenaga Kependidikan','P','2023-01-09']
  ...
  D.jenis = [
    ...
    { nama:'Staf', perlu_rombel:false, perlu_jabatan:true, piket_sekolah:'Melekat',
      ...
      penjelasan:'Jabatan struktural atau tenaga pendukung (satpam, kebersihan), bertugas penuh menurut jam kerja. Kehadiran lewat fingerprint atau absen manual; tanpa transport piket terpisah.' },
    ...
    { nama:'Piket', perlu_rombel:false, perlu_jabatan:false, piket_sekolah:'Ditugaskan',
      piket_libur:false, tambah_jam_mengajar:false, jam_unit:false, hak_transport:true,
      penjelasan:'Ditugaskan khusus menambal jam piket meja sekolah, bagi guru yang bukan wali kelas dan bukan staf — pada keduanya piket sudah melekat.' },
  ...
  D.jabatan = [
    ...
    ['Penanggung Jawab Ketertiban','Unit'], ['Satpam','Pendukung'], ['Petugas Kebersihan','Pendukung']
  ].map(([nama, kategori]) => ({ nama, kategori, aktif: true }));

  D.tugas = [
    { id:'T1', guru_id:'G001', jenis:'Wali Kelas', rombel_id:'R0', jabatan:null,
      jam_tambahan_mengajar:null, jam_piket_unit:null, jam_piket:4, mulai:'2026-07-13', aktif:true, tahun_ajaran:sesi.ta },
    { id:'T2', guru_id:'G003', jenis:'Staf', rombel_id:null, jabatan:'Wakasek Kesiswaan', pola_honor:'bulanan', sumber_hadir:'fingerprint',
      jam_tambahan_mengajar:null, jam_piket_unit:null, jam_piket:3, mulai:'2026-07-13', aktif:true, tahun_ajaran:sesi.ta },
    { id:'T7', guru_id:'G005', jenis:'Staf', rombel_id:null, jabatan:'Satpam', pola_honor:'bulanan_harian', sumber_hadir:'fingerprint',
      jam_tambahan_mengajar:null, jam_piket_unit:null, jam_piket:0, mulai:'2026-07-13', aktif:true, tahun_ajaran:sesi.ta },
    { id:'T8', guru_id:'G006', jenis:'Staf', rombel_id:null, jabatan:'Petugas Kebersihan', pola_honor:'harian', sumber_hadir:'manual',
      jam_tambahan_mengajar:null, jam_piket_unit:null, jam_piket:0, mulai:'2026-07-13', aktif:true, tahun_ajaran:sesi.ta },
    { id:'T3', guru_id:'G004', jenis:'Diperbantukan', rombel_id:null, jabatan:'Penanggung Jawab Laboratorium IPA',
      jam_tambahan_mengajar:null, jam_piket_unit:4, jam_piket:null, mulai:'2026-07-13', aktif:true, tahun_ajaran:sesi.ta },
  ...
  D.jamPel = [['07:00','07:45'],['07:45','08:30'],['08:30','09:15'],['09:30','10:15'],
              ['10:15','11:00'],['11:00','11:45'],['12:30','13:15'],['13:15','14:00']]
    .map(([mulai, selesai], i) => ({ jam_ke: i + 1, mulai, selesai }));
  D.piketJadwal = [
    ['PK1','G002','Senin',1,'Piket',false], ['PK2','G002','Senin',2,'Piket',false],
    ['PK3','G002','Selasa',1,'Piket',false], ['PK4','G002','Selasa',2,'Piket',false],
    ['PK5','G001','Rabu',1,'Wali Kelas',false], ['PK6','G001','Rabu',2,'Wali Kelas',false],
    ['PK7','G003','Kamis',4,'Staf',true], ['PK8','G003','Kamis',5,'Staf',true]
  ].map(([id, guru_id, hari, jam_ke, dasar, staf]) => ({ id, guru_id, guru: namaGuru(guru_id), hari, jam_ke, dasar, staf }));
  D.jamKerja = [['Senin','06:30','15:45'],['Selasa','06:30','15:45'],['Rabu','06:30','15:45'],
                ['Kamis','06:30','15:45'],['Jumat','06:30','14:30'],['Sabtu',null,null]]
    .map(([hari, masuk, pulang], i) => ({ hari, urutan: i + 1, aktif: !!masuk, masuk, pulang }));
  D.jamKerjaGuru = [
    { guru_id:'G003', hari:'Jumat', libur:false, masuk:'06:30', pulang:'13:30', catatan:'Kuliah sore' }
  ];
  D.parkiran = [
...
function formulir({ judul, kolom, nilai = {}, simpan, lebar, catatan, hapus }) {
  ...
  const gambarKolom = () => kolom.filter(k => !k.bila || k.bila(isi)).map(k => {
    ...
    if (k.tipe === 'info') {
      ...
      const t = k.tipe === 'tanggal' ? 'date' : k.tipe === 'waktu' ? 'time' : 'text';
...
function gambar() {
  ...
  ({ beranda: halBeranda, siswa: halSiswa, guru: halGuru, tugas: halTugas,
     kelas: halKelas, jadwal: halJadwal, kelompok: halKelompok, mapel: halMapel, jabatan: halJabatan, piket: halPiket, jamkerja: halJamKerja, profil: halProfil, tahun: halTahun }[halaman] || halBeranda)();
  gambarSelbar();
}
      ...
      <div class="scroll"><table><thead><tr>
        ...
        data.length ? data.map(g => `<tr class="${g.status_aktif !== 'Aktif' ? 'mati' : ''}" data-id="${esc(g.id)}">
          ...
          <td style="font-weight:500">${esc(g.nama)}
            ${g.status_aktif !== 'Aktif' ? `<span class="tag tag-l">${esc(g.status_aktif)}</span>` : ''}${tagBpjs(g.id)}</td>
...
function dialogBpjs(id, jenis) {
  ...
  if (b.status === 'memenuhi') {
    formulir({
      ...
      simpan: async n => {
        const mulai = String(n.mulai).slice(0, 7) + '-01';
        if (MODE === 'contoh') {
  ...
  konfirmasi({
    ...
    pesan: `<b>${esc(g.nama)}</b> — disahkan${b.disahkan_oleh ? ' oleh ' + esc(b.disahkan_oleh) : ''}${
      ...
      + (b.status === 'terhenti'
          ? `Saat ini <b>terhenti</b>: ${esc(b.keterangan || '')}. Pembayaran sudah berhenti sendiri; pengesahannya boleh dibiarkan sebagai riwayat, atau dicabut.`
          : 'Mencabut pengesahan menghentikan tunjangan; riwayat pengesahannya tetap tersimpan.'),
    lanjut: async () => {
      if (MODE === 'contoh') {
        Object.assign(b, { status: b.memenuhi ? 'memenuhi' : 'belum', mulai: null, pengesahan_id: null });
      } else {
...
/* Pola honor tenaga kependidikan — per orang, bukan per jenis. Rupiahnya di
   Induk Pembiayaan; di sini hanya siapa berpola apa dan dari mana hari
   hadirnya dihitung. */
const POLA_HONOR = [
  ['bulanan',        'Honor bulanan saja'],
  ['bulanan_harian', 'Honor bulanan + insentif kedatangan per hari hadir'],
  ['harian',         'Upah harian per kedatangan saja']
];
const SUMBER_HADIR = [['fingerprint', 'Mesin fingerprint'], ['manual', 'Absen manual oleh petugas']];
const teksPola = t => {
        ...
        data.length ? data.map(t => {
          ...
          return `<tr class="${t.aktif ? '' : 'mati'} ${kurang.length ? 'bad' : ''}" data-id="${esc(t.id)}">
            ...
            <td>${esc(rincianTugas(t))}
              ${t.jenis === 'Staf' ? `<div class="kecil">${esc(teksPola(t))}</div>` : ''}
...
function formTugas(t) {
  ...
  formulir({
    ...
    kolom: [
      ...
      { k: 'jabatan', label: 'Jabatan / unit', tipe: 'pilih', wajib: true,
        ...
        hint: D.jabatan.length
          ? 'Belum ada di daftar? Tambahkan lewat halaman Jabatan dan Unit, daftar ini langsung ikut terbarui.'
          : 'Daftar jabatan masih kosong. Isi lebih dulu lewat halaman Jabatan dan Unit.' },

      { k: 'pola_honor', label: 'Pola honor', tipe: 'pilih', pemicu: true,
        bila: n => n.jenis === 'Staf',
        opsi: POLA_HONOR.map(([v, t]) => ({ v, t })),
        hint: 'Staf pada umumnya honor bulanan saja. Staf kontrak dan satpam: bulanan + insentif kedatangan. '
            + 'Tenaga harian: upah per kedatangan. Nominalnya diisi di Induk Pembiayaan.' },
      { k: 'sumber_hadir', label: 'Hari hadir dihitung dari', tipe: 'pilih',
    ...
    simpan: async n => {
      // Piket meja sekolah sudah melekat pada Wali Kelas dan Staf, jadi jam
      // piketnya dicatat di baris itu. Tugas Piket hanya untuk guru yang
      // bukan keduanya — mencatat dua-duanya membuat jamnya terhitung ganda.
      const lain = D.tugas.filter(x => x.aktif && x.guru_id === n.guru_id && (!t || x.id !== t.id));
      const melekat = lain.find(x => sifat(x.jenis, 'piket_sekolah') === 'Melekat');
      if (n.jenis === 'Piket' && melekat)
        throw new Error(`${namaGuru(n.guru_id)} sudah tercatat ${melekat.jenis}, dan piket meja sekolah melekat `
                      + `pada tugas itu. Isi jam piketnya di baris ${melekat.jenis}, bukan sebagai tugas Piket.`);
      const piketLama = lain.find(x => x.jenis === 'Piket');
      if (sifat(n.jenis, 'piket_sekolah') === 'Melekat' && piketLama)
        throw new Error(`${namaGuru(n.guru_id)} masih tercatat tugas Piket (${piketLama.jam_piket ?? '—'} jam). `
                      + `Sebagai ${n.jenis}, piketnya melekat — hapus atau akhiri tugas Piket itu dulu, lalu isi jam piketnya di sini.`);
      const isi = {
...
/* Dua tampilan dalam satu halaman Piket Meja Sekolah. Dipakai segmented
...
function halPiketMatriks() {
  ...
  // Senin–Jumat selalu ada barisnya supaya hari yang masih kosong pun bisa
  // diisi lewat seret; Sabtu hanya bila memang ada yang berjaga.
  const pakai = HR.filter(h => h !== 'Sabtu' || data.some(p => p.hari === h));
  ...
  // Kesepakatan jam piket meja sekolah per guru: jumlah jam_piket pada tugas
  // aktif yang membawa piket meja sekolah (Wali Kelas, Staf, Piket). Dasarnya
  // disusun persis seperti v_jadwal_piket supaya warnanya sama dengan pitanya.
  const peringkat = peringkatGuru();
  const sepakat = new Map();
  D.tugas.filter(t => t.aktif && sifat(t.jenis, 'piket_sekolah')).forEach(t => {
    const g = D.guru.find(x => x.id === t.guru_id);
    if (!g || g.status_aktif !== 'Aktif') return;
    const x = sepakat.get(t.guru_id) || { jam: 0, ada: false, jenis: new Set() };
    if (t.jam_piket != null) { x.jam += Number(t.jam_piket) || 0; x.ada = true; }
    x.jenis.add(t.jenis);
    sepakat.set(t.guru_id, x);
  });
  const dasarGuru = id => sepakat.has(id) ? [...sepakat.get(id).jenis].sort().join(' + ') : 'belum tercatat';
  const ditempatkan = id => data.filter(p => p.guru_id === id).length;
  // Guru yang ada di jadwal tetapi tidak punya dasar penugasan ikut ditampilkan,
  // supaya daftar sisa menyebut semua orang yang ada di matriks.
  const tanpaDasar = [...new Set(data.map(p => p.guru_id))].filter(id => !sepakat.has(id));
  ...
  const dasarAda = [...new Set([...data.map(p => p.dasar), ...[...sepakat.keys()].map(dasarGuru)])].sort((a, b) =>
  ...
  const barisHtml = pakai.map(h => {
    ...
    const tds = kolom.map((c, ci) => {
      ...
      const pita = !n ? '' : jalur.map(lajur => {
        ...
        // Jam berturut-turut tampil sebagai satu pita panjang; supaya tidak
        // terbaca sebagai satu jam, pita yang melintasi beberapa kolom
        // menyebut panjangnya di samping nama.
        let panjang = 1;
        if (!sambungKiri) for (let k = ci + 1; k < kolom.length && !kolom[k].sela && ada(g, kolom[k].jk); k++) panjang++;
        return `<div class="pk-pita${sambungKiri ? ' kiri' : ''}${sambungKanan ? ' kanan' : ''}${p.staf ? ' staf' : ''}"
          ...
          title="${esc(p.guru + '\n' + h + ' jam ke-' + c.jk + (jamTeks(c.jk) ? ' (' + jamTeks(c.jk) + ')' : '')
                 ...
                 + '\nSeret untuk memindahkan')}">${esc(namaPendek(p.guru))}${
                 panjang > 1 ? `<small>${panjang} jam</small>` : ''}</div>`;
      }).join('');
  ...
  // Sisa jam yang belum ditempatkan, siap diseret ke matriks — sama seperti
  // matriks piket unit. Urutannya mengikuti masa kerja.
  const urutMasaKerja = (a, b) => peringkat(a) - peringkat(b);
  const sisaHtml = [...sepakat.keys()].sort(urutMasaKerja).map(id => {
    const x = sepakat.get(id), w = warnaDasar(dasarGuru(id)), nama = namaGuru(id), sudah = ditempatkan(id);
    const judul = nama + '\n' + dasarGuru(id);
    if (!x.ada) return `<span class="pu-sisa lebih" style="--bg:${w[0]};--aksen:${w[1]}"
      title="${esc(judul + '\nJam piketnya belum diisi di Tugas Guru')}">${esc(namaPendek(nama))} <b>belum disepakati</b></span>`;
    const sisa = x.jam - sudah;
    if (sisa <= 0) return `<span class="pu-sisa ${sisa < 0 ? 'lebih' : 'penuh'}" style="--bg:${w[0]};--aksen:${w[1]}"
      title="${esc(judul + '\n' + sudah + ' dari ' + x.jam + ' jam ditempatkan')}">${esc(namaPendek(nama))} <b>${sisa < 0 ? 'lebih ' + (-sisa) + ' jam' : 'pas'}</b></span>`;
    return `<span class="pu-sisa" draggable="true" data-guru="${esc(id)}" style="--bg:${w[0]};--aksen:${w[1]}"
      title="${esc(judul + '\nSisa ' + sisa + ' dari ' + x.jam + ' jam — seret ke matriks')}">${esc(namaPendek(nama))} <b>${sisa} jam</b></span>`;
  }).concat(tanpaDasar.sort(urutMasaKerja).map(id => {
    const w = warnaDasar('belum tercatat'), nama = namaGuru(id);
    return `<span class="pu-sisa lebih" style="--bg:${w[0]};--aksen:${w[1]}"
      title="${esc(nama + '\nAda di jadwal, tetapi dasar penugasannya belum tercatat di Tugas Guru')}">${esc(namaPendek(nama))} <b>${ditempatkan(id)} jam · tanpa dasar</b></span>`;
  })).join('');
  const belumSepakat = [...sepakat.values()].filter(x => !x.ada).length;
  const lebih = [...sepakat.entries()].filter(([id, x]) => x.ada && ditempatkan(id) > x.jam).length;
  const totalSepakat = [...sepakat.values()].reduce((a, x) => a + x.jam, 0);
  ...
  $('#isi').innerHTML = `
    ...
    ${!data.length && !D.jamPel.length ? `<div class="panel"><div class="empty"><b>Belum ada jam pelajaran maupun jadwal piket terbaca</b>
      Isi daftar jam pelajaran lebih dulu, lalu muat ulang halaman.</div></div>` : `

    ${belumSepakat ? `<div class="info-box"><b>${belumSepakat} guru belum menyebut jam piket meja sekolah per minggu.</b>
      Isi dulu di halaman Tugas Guru — tanpa angka itu, sisa jamnya tidak bisa dihitung.</div>` : ''}
    ${lebih ? `<div class="info-box"><b>${lebih} guru dijadwalkan melebihi jam yang disepakati.</b>
      Yang dibayar tetap mengikuti jadwal dan pelaksanaannya; samakan angka kesepakatan di Tugas Guru bila memang berubah.</div>` : ''}
    ...
    <div class="panel"><div class="panel-head"><h3>Sisa jam belum ditempatkan</h3>
      ...
      <div class="pu-tray" id="pkTray">${sisaHtml ||
        '<span class="kecil">Belum ada guru yang tercatat bertugas piket meja sekolah di Tugas Guru.</span>'}</div>
      <div class="foot"><div class="info">Seret pita dari matriks ke kotak ini untuk mengeluarkannya dari jadwal.</div></div>
    </div>
    ...
    <div class="foot"><div class="info">${data.length} jam terjadwal ·
      ...
      ${new Set(data.map(p => p.hari)).size} hari ·
      ${totalSepakat} jam disepakati</div></div></div>

    <p class="kecil">Jam kesepakatan tiap guru diisi di <b>Tugas Guru</b>; daftar sisa di atas
      membandingkannya dengan yang sudah ditempatkan. Seret nama dari daftar sisa ke sel untuk
      menambah jam jaganya, atau ketuk sel mana pun untuk mengatur lewat daftar.
      Tiap petugas punya lajurnya sendiri dalam satu hari, jadi jam jaga yang berturut-turut
      tampil sebagai satu pita panjang. Warna pita menunjukkan atas dasar apa ia berjaga;
      pita bergaris putus-putus berarti staf (tanpa transport). Kolom berlatar abu belum ada petugasnya.
  ...
  pasangTabPiket();
  // Berkas unduhan hanya memuat hari yang memang ada petugasnya.
  if ($('#bUnduhPiket')) $('#bUnduhPiket').onclick = () =>
    unduhPiketMejaXlsx(HR.filter(h => data.some(p => p.hari === h)), jamKe, sel, jamTeks);
  pasangSeretMeja();
}
...
function pasangSeretMeja() {
  let bawa = null;                       // { id } dari matriks, atau { guruId } dari daftar sisa
  ...
  $$('.pk-pita[data-id]').forEach(el => mulai(el, { id: el.dataset.id }));
  $$('.pu-sisa[data-guru]').forEach(el => mulai(el, { guruId: el.dataset.guru }));
  ...
  $$('.pk-sel').forEach(td => {
    ...
    td.addEventListener('drop', ev => {
      ...
      if (!m) return;
      if (m.id) pindahPiketMeja(m.id, td.dataset.hari, Number(td.dataset.jam));
      else if (m.guruId) tambahPiketMeja(m.guruId, td.dataset.hari, Number(td.dataset.jam));
    });
    td.addEventListener('click', () => dialogPiketSel(td.dataset.hari, +td.dataset.jam));
  });

  // Daftar sisa di atas dan kotak buang di bawah sama-sama menerima pita
  // untuk dikeluarkan dari jadwal.
  [$('#pkTray'), $('#pkBuang')].filter(Boolean).forEach(buang => {
...
/* Menambahkan satu jam jaga. Dipakai dialog sel dan seret dari daftar sisa. */
function tambahPiketMeja(guruId, hari, jamKe) {
  jalankan('Menyimpan…', async () => {
    const sebab = bentrokMeja(guruId, hari, jamKe, null);
    if (sebab) throw new Error(`${namaGuru(guruId)} ${sebab} pada ${hari} jam ke-${jamKe}.`);
    if (MODE === 'contoh') {
      const jenis = [...new Set(D.tugas.filter(t => t.aktif && t.guru_id === guruId && sifat(t.jenis, 'piket_sekolah'))
                                       .map(t => t.jenis))].sort();
      D.piketJadwal.push({ id: 'PK' + Date.now(), guru_id: guruId, guru: namaGuru(guruId), hari, jam_ke: jamKe,
                           dasar: jenis.length ? jenis.join(' + ') : 'belum tercatat', staf: jenis.includes('Staf') });
    } else {
      await simpanBaru('piket', {
        id: 'PK' + Date.now().toString(36).toUpperCase(),
        guru_id: guruId, hari, jam_ke: jamKe });
      await muatSemua();
    }
    toast(`${namaPendek(namaGuru(guruId))}: ${hari} jam ke-${jamKe}`);
  });
}
...
function dialogPiketSel(hari, jamKe) {
  ...
  $('#m-tambah').onclick = () => {
    ...
    tutupModal();
    tambahPiketMeja(guruId, hari, jamKe);
  };
}
    ...
    const sel_ = sel.map(s => {
      ...
      if (s.ringkas) {
        ...
        return `<td class="mx-sel${skr}"><div class="mx-blok ringkas${sambung}" data-lingkup="${esc(b.grup)}"
          ...
          <span class="u">Kelompok ${esc(b.grup)}</span><span class="k">${esc(ket)} ›${jam}</span></div></td>`;
      }
      if (s.kunci) return `<td class="mx-sel${skr}"><div class="mx-blok kunci${sambung}" style="--rentang:${s.rentang}"
          ...
          <span class="u">${esc(s.kunci.join(' · '))}</span><span class="k">jam kelompok${jam}</span></div></td>`;
      ...
      const blok = s.daftar.map(j => {
        ...
        return `<div class="mx-blok${sambung}${banyak ? ' rapat' : ''}" data-jid="${esc(j.id)}" ${gaya(warnaMapel(j.mapel))} title="${esc(judul)}">
          <span class="u">${esc(utama(j))}</span><span class="k">${esc(kedua(j))}${jam}</span></div>`;
      }).join('');
...
function halPiket() {
  ...
  if ((ui.mejaSub || 'matriks') !== 'petugas') return halPiketMatriks();
...
/* ------------------------------------------------------------ kelas */
/* ------------------------------------------------- jam kerja staf */
/* Ketentuan jam kerja bagi pemegang tugas Staf: bawaan sekolah per hari,
   ditambah pengecualian per orang (libur pada hari tertentu, atau jam yang
...
function kelompokTarifBawaan(jabatan, jenisPtk) {
  const j = (jabatan || '').toLowerCase();
  // Jenis PTK Pimpinan selalu kelompok pimpinan, apa pun jabatan tugas Staf-nya (25 September 2026).
  if (jenisPtk === 'Pimpinan') return j.startsWith('kepala sekolah') ? 'kepala_sekolah' : 'wakasek';
  if (!j) return 'staf';
...
const kelompokTarifDari = (t, g) => (t && t.kelompok_tarif) || kelompokTarifBawaan(t && t.jabatan, g && g.jenis_ptk);
...
const bawaanHari = h => D.jamKerja.find(x => x.hari === h) || { hari: h, aktif: false, masuk: null, pulang: null };
const kecualiHari = (guruId, h) => D.jamKerjaGuru.find(r => r.guru_id === guruId && r.hari === h);
// Jam kerja efektif seorang staf pada satu hari: bawaan, khusus, atau libur.
function efektifHari(guruId, h) {
  const b = bawaanHari(h), x = kecualiHari(guruId, h);
  if (!x) return { sumber: 'bawaan', bekerja: !!b.aktif, masuk: b.masuk, pulang: b.pulang };
  if (x.libur) return { sumber: 'libur', bekerja: false, masuk: null, pulang: null, catatan: x.catatan };
  return { sumber: 'khusus', bekerja: true, masuk: x.masuk, pulang: x.pulang, catatan: x.catatan };
}
const lamaHari = e => e.bekerja ? menitDari(e.pulang) - menitDari(e.masuk) : 0;
const menitMinggu = ambil => HARI_KERJA.reduce((a, h) => a + lamaHari(ambil(h)), 0);
// Pimpinan (jenis PTK) ikut tunduk walau tidak memegang tugas Staf, sama seperti v_jam_kerja_guru.
const stafTunduk = () => D.guru.filter(g => g.status_aktif === 'Aktif' && (stafDari(g.id) || g.jenis_ptk === 'Pimpinan'));
const rentangJam = e => e.bekerja ? jam5(e.masuk) + '–' + jam5(e.pulang) : 'libur';

function halJamKerja() {
  const staf = stafTunduk();
  const bawaan = h => efektifHari('__bawaan__', h);
  const hariAktif = HARI_KERJA.filter(h => bawaanHari(h).aktif).length;

  // Skala pita waktu: dari jam masuk paling pagi sampai jam pulang paling
  // sore di seluruh ketentuan, dibulatkan ke jam penuh.
  const semua = [...D.jamKerja.filter(x => x.aktif), ...D.jamKerjaGuru.filter(x => !x.libur)];
  const a = Math.floor(Math.min(6 * 60, ...semua.map(x => menitDari(x.masuk))) / 60) * 60;
  const b = Math.ceil(Math.max(16 * 60, ...semua.map(x => menitDari(x.pulang))) / 60) * 60;
  const pita = e => {
    if (!e.bekerja) return '<div class="jk-bar libur"></div>';
    const kiri = (menitDari(e.masuk) - a) / (b - a) * 100, lebar = lamaHari(e) / (b - a) * 100;
    return `<div class="jk-bar"><i style="left:${kiri}%;width:${lebar}%"></i></div>`;
  };
  const skala = [];
  for (let m = a; m <= b; m += 120) skala.push(teksMenit(m));
  ...
  $('#isi').innerHTML = `
    ...
    <div class="kartu-baris">
      <div class="kartu"><b>${staf.length}</b><span>staf tunduk jam kerja</span></div>
      <div class="kartu"><b>${hariAktif}</b><span>hari kerja per minggu</span></div>
      <div class="kartu"><b>${teksDurasi(menitMinggu(bawaan))}</b><span>jam kerja bawaan per minggu</span></div>
      <div class="kartu"><b>${D.jamKerjaGuru.length}</b><span>pengecualian per orang</span></div>
    </div>

    <div class="panel"><div class="panel-head"><h3>Ketentuan bawaan sekolah</h3>
      <div class="sp" style="flex:1"></div><div class="info">ketuk hari untuk mengubah</div></div>
      <div class="jk-grid">${HARI_KERJA.map(h => {
        const e = bawaan(h);
        return `<button class="jk-hari${e.bekerja ? '' : ' libur'}" data-hari="${h}"
          title="${esc(h + ': ' + (e.bekerja ? jam5(e.masuk) + ' – ' + jam5(e.pulang) : 'libur') + '\nKetuk untuk mengubah')}">
          <b>${h}</b>
          <span class="jk-jam">${e.bekerja ? jam5(e.masuk) + ' – ' + jam5(e.pulang) : 'Libur'}</span>
          ${pita(e)}
          <small>${e.bekerja ? teksDurasi(lamaHari(e)) : 'tidak ada jam kerja'}</small></button>`;
      }).join('')}</div>
      <div class="jk-skala">${skala.map(t => `<span>${t}</span>`).join('')}</div>
    </div>

    <div class="panel"><div class="panel-head"><h3>Staf yang tunduk</h3>
      <div class="sp" style="flex:1"></div><div class="info">dari tugas Staf aktif di Tugas Guru</div></div>
      <div class="scroll"><table><thead><tr>
        ...
        staf.length ? staf.map(g => {
          const t = stafDari(g.id) || {};   // Pimpinan boleh tanpa tugas Staf
          ...
          return `<tr data-id="${esc(g.id)}">
            <td style="font-weight:500">${esc(g.nama)}<div class="kecil">${esc(t.jabatan || (g.jenis_ptk === 'Pimpinan' ? 'Pimpinan (tanpa tugas Staf)' : '—'))}${
              t.sumber_hadir === 'manual' ? ' · absen manual' : ''}${nKhusus ? ` · ${nKhusus} hari berbeda dari bawaan` : ''}</div></td>
            ${HARI_KERJA.map(h => {
              const e = efektifHari(g.id, h);
            ...
            <td>${esc(namaKelompokTarif(kelompokTarifDari(t, g)))}${t.kelompok_tarif ? '' : `<div class="kecil">${g.jenis_ptk === 'Pimpinan' ? 'bawaan: jenis PTK Pimpinan' : 'bawaan menurut jabatan'}</div>`}</td>
      ...
      <div class="foot"><div class="info">
        <span class="jk-legenda bawaan">ikut bawaan</span>
        <span class="jk-legenda khusus">jam khusus</span>
        <span class="jk-legenda libur">libur khusus</span></div></div>
    </div>
  ...
  $$('tbody tr[data-id] .bAtur').forEach(b => b.onclick = () => {
    const g = D.guru.find(x => x.id === b.closest('tr').dataset.id);
    if (g) dialogJamKerjaGuru(g);
  });
}

function formJamKerjaHari(hari) {
  const b = bawaanHari(hari);
  formulir({
    judul: 'Jam kerja bawaan — ' + hari,
    nilai: { aktif: b.aktif ? 'ya' : 'tidak', masuk: jam5(b.masuk) || '06:30', pulang: jam5(b.pulang) || '15:45' },
    kolom: [
      { k: 'aktif', label: 'Hari kerja', tipe: 'pilih', pemicu: true,
        opsi: [{ v: 'ya', t: 'Ya — staf bekerja pada hari ini' }, { v: 'tidak', t: 'Tidak — libur kerja' }] },
      { k: 'masuk',  label: 'Jam masuk',  tipe: 'waktu', wajib: true, bila: n => n.aktif === 'ya' },
      { k: 'pulang', label: 'Jam pulang', tipe: 'waktu', wajib: true, bila: n => n.aktif === 'ya',
        hint: 'Berlaku bagi semua staf yang tidak punya pengecualian pada hari ini.' }
    ],
    simpan: async n => {
      const aktif = n.aktif === 'ya';
      if (aktif && menitDari(n.pulang) <= menitDari(n.masuk)) throw new Error('Jam pulang harus sesudah jam masuk.');
      const isi = { aktif, masuk: aktif ? n.masuk : null, pulang: aktif ? n.pulang : null };
      if (MODE === 'db') await perbarui('jam_kerja', `hari=eq.${enc(hari)}`, isi);
      Object.assign(b, isi);
      if (!D.jamKerja.includes(b)) D.jamKerja.push({ urutan: HARI_KERJA.indexOf(hari) + 1, ...b });
      toast(hari + ': ' + (aktif ? n.masuk + '–' + n.pulang : 'libur'));
    }
  });
}

/* Pengecualian per orang: tiap hari boleh ikut bawaan, jam khusus, atau libur.
   Satu tabel kecil, bukan formulir umum, supaya enam hari terlihat sekaligus. */
function dialogJamKerjaGuru(g) {
  ...
  const bawaanKelompok = kelompokTarifBawaan(tugas && tugas.jabatan, g.jenis_ptk);

  bukaModal(`<h2>Jam kerja — ${esc(g.nama)}</h2><div class="body">
    <div class="fg"><label>Kelompok tarif honor</label>
      <select class="field" id="jkKelompok" ${tugas ? '' : 'disabled'}>
      ...
      <div class="hint">Menentukan tunjangan jabatan dan indeks di Induk Pembiayaan → Nominal Penggajian Staf.
        Jabatan: ${esc((tugas && tugas.jabatan) || '—')}.${tugas ? '' : ' Tanpa tugas Staf, kelompoknya mengikuti jenis PTK dan tidak bisa ditetapkan di sini.'}</div></div>
    ...
    <table class="log jk-atur"><thead><tr>
      <th>Hari</th><th style="width:150px">Ketentuan</th><th style="width:105px">Masuk</th><th style="width:105px">Pulang</th>
    </tr></thead><tbody>${HARI_KERJA.map(h => {
      const b = bawaanHari(h), x = kecualiHari(g.id, h);
      const mode = !x ? 'bawaan' : x.libur ? 'libur' : 'khusus';
      return `<tr data-hari="${h}">
        <td><b>${h}</b><div class="kecil">bawaan: ${b.aktif ? jam5(b.masuk) + '–' + jam5(b.pulang) : 'libur'}</div></td>
        <td><select class="field" data-mode>${MODE_JK.map(([v, t]) =>
          `<option value="${v}" ${v === mode ? 'selected' : ''}>${t}</option>`).join('')}</select></td>
        <td><input class="field" type="time" data-masuk value="${mode === 'khusus' ? jam5(x.masuk) : ''}" ${mode === 'khusus' ? '' : 'disabled'}></td>
        <td><input class="field" type="time" data-pulang value="${mode === 'khusus' ? jam5(x.pulang) : ''}" ${mode === 'khusus' ? '' : 'disabled'}></td></tr>`;
    }).join('')}</tbody></table>
    <div class="fg" style="margin-top:14px"><label>Keterangan</label>
      <input class="field" id="jkCatatan" value="${esc(catatanLama)}" autocomplete="off">
      <div class="hint">Misalnya "Kontrak Senin–Jumat, masih studi". Disimpan pada tiap hari yang dikecualikan.</div></div>
    </div>
    <div class="aksi"><button class="btn" id="m-batal">Batal</button>
      <button class="btn btn-p" id="m-simpan">Simpan</button></div>`, true);

  $$('#modal-root [data-mode]').forEach(sel => sel.onchange = () => {
    const tr = sel.closest('tr'), b = bawaanHari(tr.dataset.hari), khusus = sel.value === 'khusus';
    const m = $('[data-masuk]', tr), p = $('[data-pulang]', tr);
    m.disabled = p.disabled = !khusus;
    if (khusus && !m.value) { m.value = jam5(b.masuk) || '06:30'; p.value = jam5(b.pulang) || '15:45'; }
    if (!khusus) { m.value = ''; p.value = ''; }
  });
  $('#m-batal').onclick = tutupModal;
  $('#m-simpan').onclick = () => {
    ...
    for (const tr of $$('#modal-root tr[data-hari]')) {
      ...
      if (mode === 'bawaan') continue;
      if (mode === 'libur') { baris.push({ guru_id: g.id, hari, libur: true, masuk: null, pulang: null, catatan }); continue; }
      const masuk = $('[data-masuk]', tr).value, pulang = $('[data-pulang]', tr).value;
      if (!masuk || !pulang) { toast(hari + ': isi jam masuk dan pulang.', true); return; }
      if (menitDari(pulang) <= menitDari(masuk)) { toast(hari + ': jam pulang harus sesudah jam masuk.', true); return; }
      baris.push({ guru_id: g.id, hari, libur: false, masuk, pulang, catatan });
    }
    tutupModal();
    jalankan('Menyimpan…', async () => {
      if (MODE === 'db') {
...
function halKelas() {
  ...
  $('#isi').innerHTML = `
    <div class="head"><div><h1>Kelas dan Rombel</h1><p>Rombongan belajar tahun ajaran ${esc(sesi.ta)}.</p></div>
...
function halJabatan() {
  ...
  $('#isi').innerHTML = `
    <div class="head"><div><h1>Jabatan dan Unit</h1>
      <p>Pilihan yang tersedia saat mencatat tugas Staf dan Diperbantukan. Tenaga pendukung
         (satpam, kebersihan) dicatat sebagai Staf dengan jabatan berkategori Pendukung.</p></div>
    ...
    kolom: [
      ...
      { k: 'kategori', label: 'Kategori', tipe: 'pilih',
        opsi: [{ v: 'Struktural', t: 'Struktural — jabatan ber-SK' },
               ...
               { v: 'Pendukung', t: 'Pendukung — satpam, kebersihan, tenaga harian' }] },
...
function unduhCadangan() {
  ...
  tambah('Tugas', [['Guru', 'namaGuru'], ['ID Guru', 'guru_id'], ['Jenis', 'jenis'], ['Kelas', 'kelas'],
                   ...
                   ['Jam piket unit', 'jam_piket_unit'], ['Jam piket meja sekolah', 'jam_piket'],
                   ['Pola honor', 'pola_honor'], ['Sumber hadir', 'sumber_hadir'],
                   ['Mulai', 'mulai'], ['Aktif', 'aktif'],
